...
function ProductDetail() {
  ...
  if (error || !product) {
    return (
      <div className="rounded-2xl border border-dashed border-primary-200 bg-white p-10 text-center">
        <p className="text-primary-500">{error || 'Producto no encontrado.'}</p>
        <Link
          to="/"
  ...
  return (
    <div className="space-y-5">
      <Link
        to="/"
        className="inline-flex items-center gap-1.5 text-sm font-medium text-primary-600 transition hover:text-primary-800"
      >
        <svg
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-primary-100 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-medium text-primary-500">{product.code}</p>
          <h1 className="text-2xl font-bold text-primary-900">{product.name}</h1>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <div className="text-right">
            <p className="text-xs text-primary-400">Stock actual</p>
            <p
              className={`text-2xl font-bold ${
                product.current_stock > 0 ? 'text-emerald-600' : 'text-red-500'
              }`}
            >
